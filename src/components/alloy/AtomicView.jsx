export default function AtomicView({ selectedMaterial, show, onToggle, learningMessage }) {
  ...
  return (
    <section className="electroPanel alloyAtomicPanel">
      <div className="alloyPanelHeader">
        <h2>Atomic View</h2>
        <button className="alloyAtomToggle" type="button" onClick={onToggle} aria-pressed={show}>
          <span>{show ? "Sembunyikan atom" : "Buka paparan atom"}</span>
        </button>
      </div>
      ...
      {show ? (
        <div className="alloyAtomicContent">
          ...
          <div className="alloyAtomicExplanation">
            <h3>Apa yang berlaku?</h3>
            <p>{learningMessage}</p>
          </div>
        </div>
      ) : (
        <p className="alloyMuted alloyAtomicPrompt">Tekan butang biru di atas untuk melihat susunan atom dan kaitannya dengan kekerasan bahan.</p>
      )}
    </section>
  );
}
